...
**Non-blocking rule:** the SocketCAN receive loop must never block on core, SQLite, MQTT, HTTP, dashboard, or AI services. All downstream paths use bounded queues. On overload: record lag, spool to the bounded log, mark ingestion degraded, count/expose dropped frames — **known or inferred loss is surfaced and degrades data-confidence, never passed off as healthy** (some loss may be undetectable — ADR-005/012).
...
**Health model — two separate axes** (ADR-012): core computes two fields per snapshot plus a conservative summary; consumers render, never re-derive. **Battery condition** (`unknown`/`ok`/`concern`/`fault`) is asserted **only from sufficient fresh data** — missing/stale critical inputs leave it `unknown`, **never** `fault`. **Data confidence** (`ok`/`degraded`/`fault`) reflects whether we can see the pack (gaps, staleness, profile/decoder health), never pack values. The **conservative summary** is the worse of the two, tagged with a **reason domain** (`battery`/`monitoring`/`configuration`/`decoder`), so `fault(monitoring)` — "can't see the pack" — is never confused with `fault(battery)`. `healthy` needs both axes `ok` (complete, fresh critical inputs). Any gap invalidates `healthy`; a critical/prolonged gap → `fault(monitoring)` with battery `unknown`; recovery needs a fresh broadcast cycle plus dwell. Toward `healthy` uses hysteresis; toward any `fault` is immediate.
