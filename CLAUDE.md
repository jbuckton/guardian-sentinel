...
**Replayable ingestion log** (ADR-005): `guardian-can` keeps a bounded, rotated, compressed log of typed events — a **first-class replay source**, not an archive, and the **canonical record**. Durability gates visibility: the receive loop enqueues to the log writer (queue admission is *not* durability), and the log writer releases an event to IPC **only after a confirmed durable append** — so every event core sees over IPC is already durable, and IPC is a fast path, never a second source of truth. When the log itself is the failed path (disk-full/stall), lost sequence ranges are captured in a pre-allocated **emergency journal** and surface as synthesised gap/overflow events; loss is never silent. On restart core resumes from `min(safety checkpoint, telemetry watermark)` and drains via **iterative log-backed catch-up** until within a live threshold (never assuming the IPC buffer survives a long replay). **Replay and live share one decoder path — no separate replay code path is permitted.** Evidence pinning uses a **core→can reverse control channel** (pin request → durable ack → deterministic, quota-bounded eviction), reconciled at startup so a lost pin becomes an evidence-health fault. This log *is* the test harness; recorded Jr 2 sessions live in `traces/` in this format. Raw CAN frames are **never** synchronously inserted into SQLite.

**Event ordering & delivery** (ADR-004, ADR-011): a session's identity is a **durable session generation** (a monotonic counter persisted by `guardian-can`), **not order and not log position**. Sequence numbers reset per session, so the durable event identity is `(session generation, sequence number)` — stable across log rotation, prefix eviction, trace export, and restart. Order **within** a session is by sequence; order **across** sessions is by **ascending session generation** (the log's append order realises it), never by comparing timestamps or sequence numbers across sessions. Monotonic time orders only within a boot; wall-clock carries a confidence qualifier (`unsynchronised`/`synchronised`/`stepped`), and receipt time ≠ physical-event time. Delivery is **at-least-once with idempotent, event-identity-keyed effect application** — never assume exactly-once; write all effects (telemetry, findings, alert transitions, uplink) to be idempotent. Core keeps **two durable positions**: a safety checkpoint (advances after required durable safety effects) and a telemetry watermark (advances after a telemetry batch commits); it resumes from `min(...)` of the two so no telemetry is lost, and the checkpoint may lag telemetry batching by design.

**Persistence** (ADR-006, ADR-007, ADR-013): SQLite, WAL mode, **two separate database files** (not one file with multiple writers). **Operational** DB (config, versions, alerts, incidents, findings, outbound delivery state, replay checkpoint, evidence-pin references, device-health) vs **telemetry** DB (normalised telemetry + observations, batched/downsampled). **Single owning writer per database file** — all other components, including MQTT delivery-state updates, submit through the owner's bounded queue; no independent writers. Batched writes for telemetry; immediate durable writes for findings, alert transitions, bus-off, and config changes. Every storage consumer (log, pinned evidence, both DBs, MQTT backlog) has a **hard quota**; a **reserve** is held that only *current* operational safety state and alert uplinks may use. Exhaustion evicts in a fixed safety order (telemetry → routine log tail → closed-incident evidence → low-priority backlog) with recorded eviction/gap events. History is kept bounded by **compaction** (closed incidents → summaries, superseded transitions collapsed, acked items eligible) so the current-safety working set fits the reserve; the "never evicted" guarantee holds for that bounded set, and in the extreme terminal case loss is explicit, prioritised, and alertable — never a silent stall (ADR-013). Ingestion never stalls for storage.
