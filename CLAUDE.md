...
**Replayable ingestion log** (ADR-005): `guardian-can` keeps a bounded, rotated, compressed log of typed events — a **first-class replay source**, not an archive, and the **canonical record** (an event is appended to the log *before* it is offered to IPC; IPC is a fast path, not a second source of truth). Core checkpoints its processed event identity durably; on restart it requests events after the checkpoint, `guardian-can` replays them up to an **end-of-replay watermark**, then hands off to live. Expired ranges produce explicit **ingestion-gap events**. **Replay and live share one decoder path — no separate replay code path is permitted.** Evidence pinning uses a **core→can reverse control channel** (pin request → durable ack → deterministic, quota-bounded eviction). This log *is* the test harness; recorded Jr 2 sessions live in `traces/` in this format. Raw CAN frames are **never** synchronously inserted into SQLite.

**Event ordering & delivery** (ADR-004, ADR-011): a session ID is **identity, not order** — sequence numbers reset per session, so the durable event identity is `(session ordinal in log, sequence number)`; order across sessions is log-append order, not timestamp or sequence comparison. Monotonic time orders only within a boot; wall-clock carries a confidence qualifier (`unsynchronised`/`synchronised`/`stepped`), and receipt time ≠ physical-event time. Delivery is **at-least-once with idempotent, event-identity-keyed effect application** — never assume exactly-once; write all effects (telemetry, findings, alert transitions, uplink) to be idempotent. The checkpoint advances **only after required durable (safety-relevant) effects commit**, and may lag telemetry batching by design.

**Persistence** (ADR-006, ADR-007, ADR-013): SQLite, WAL mode, **two separate database files** (not one file with multiple writers). **Operational** DB (config, versions, alerts, incidents, findings, outbound delivery state, replay checkpoint, evidence-pin references, device-health) vs **telemetry** DB (normalised telemetry + observations, batched/downsampled). **Single owning writer per database file** — all other components, including MQTT delivery-state updates, submit through the owner's bounded queue; no independent writers. Batched writes for telemetry; immediate durable writes for findings, alert transitions, bus-off, and config changes. Every storage consumer (log, pinned evidence, both DBs, MQTT backlog) has a **hard quota**; a **reserve** is held that only operational safety state and alert uplinks may use; exhaustion evicts in a fixed safety order (telemetry first, safety state never) with recorded eviction/gap events. Ingestion never stalls for storage.
