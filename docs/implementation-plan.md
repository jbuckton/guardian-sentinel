...
- **Tiered durability (ADR-005):** Tier 0 stream/telemetry best-effort (lossy); Tier 1 incident evidence via RAM **pre-roll persisted on trigger** (best-effort); Tier 2 operational safety state (alert/incident, acks, config, decoder/profile version) **durable on change**. **Idempotency is best-effort first cut** (ADR-011) — dedup on uplink token and alert-state; strict idempotency and durable resume are a later ADR
- **Restart/resume (MVP):** `guardian-core` restart → resume from lightweight last-seen marker, re-read the still-buffered window through the same decoder path, un-buffered portion is an **explicit marked gap** that degrades health/data-confidence (no zero-loss claim); restart-mid-stream and buffer-overflow-→-gap tests are standing regressions. *(Zero-loss/durable ingestion tier is a future ADR.)*
