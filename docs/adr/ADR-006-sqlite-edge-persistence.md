...
- **Telemetry is best-effort and batched (Tier 0)** — batched transactions, downsampling tiers, bounded retention (limits are configuration). Loss is permitted; known/inferred loss surfaces as a gap that degrades data-confidence (ADR-005/012), some may be undetectable; it is never `fsync`ed per row.
